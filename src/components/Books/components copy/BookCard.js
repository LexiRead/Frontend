...
const BookCard = ({ book }) => { 
  ...
  const defaultImageOnError = 'https://via.placeholder.com/200x300.png?text=Load+Error'; // صورة إذا فشل تحميل imageUrl
        ...
        <div className="book-card-info">
          ...
          {book.categories && Array.isArray(book.categories) && book.categories.length > 0 && (
            <div className="book-card-tags">
              {book.categories.slice(0, 3).map((cat, index) => ( // عرض أول 3 وسوم مثلاً
                <span key={`${cat}-${index}`} className="book-card-tag">{cat}</span>
              ))}
            </div>
          )}
        </div>
