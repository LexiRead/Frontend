...
function UserDocumentsSection() {
  ...
  if (isLoading) return (
    <section className="books-section-container" style={{ padding: '0px' }}>
      <div className='books-section-header'><h2>Books</h2></div>
      <p>Loading your documents...</p>
    </section>
  );

  if (error && userDocuments.length === 0) return (
    <section className="books-section-container" style={{ padding: '0px' }}>
      <div className='books-section-header'><h2>My Books</h2></div>
  ...
  return (
    <section className="books-section-container" style={{ padding: '20px'}}>
      <div className='books-section-header' style={{
        display: 'flex',
      ...
      <div className="books-row-layout" style={{
        display: 'flex',
        overflowX: 'auto',
        gap: '15px',
        paddingBottom: '10px',
        ...
        {userDocuments.map(book => {
          ...
          console.log(book);
          return (
            <div key={book.id} style={{ width: '300px', flexShrink: 0 , position: "relative"}}>
              <Link to={`${isInSection ? `/book2/${book.id}` : `/book2/${book.id}`}`} style={{ textDecoration: 'none' }}>
                <BookCard book={bookCardProps} />
              </Link>

              <button
                onClick={() => handleDelete(book.id)}
                className="delete"
                style={{ width: "20px",padding: "2px",backgroundColor: "gray", color: "white", borderRadius: "50px", position: "absolute" ,left: "250px", bottom: "399px" }}
              >
                X
              </button>
