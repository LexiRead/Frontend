...
const OCR = () => {
  ...
  const handleUpload = async () => {
    if (!selectedImage) {
      setMessage("يرجى اختيار صورة للرفع.");
      return;
    }

    setLoading(true);
    setMessage("جاري الرفع والترجمة...");

    const formData = new FormData();
    formData.append("image", selectedImage);
    formData.append("target", displayLang.target);
    ...
    if (!authToken) {
      setMessage("لم يتم العثور على رمز المصادقة. يرجى تسجيل الدخول.");
      setLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch("http://app.elfar5a.com/api/ocr/translate", {
        ...
        headers: {
          ...
          "Authorization": `Bearer ${authToken}`, // استخدام الـ token من localStorage
        },
      });
      ...
      if (data && data.translated_text) {
        const translated = data.translated_text || "لا توجد ترجمة مقدمة.";
        setTranslatedTextForImage2(translated);
        setMessage("تم Conception complete!");
      } else {
        setMessage("لم يتم استلام بيانات الترجمة من الخادم.");
      }
    } catch (error) {
      console.error("Error:", error);
      if (error.message.includes("HTTP error! status: 401")) {
        setMessage("غير مصرح (401). يرجى التحقق من رمز المصادقة.");
      } else if (error.message.includes("HTTP error! status: 422")) {
        setMessage("محتوى غير قابل للمعالجة (422). يرجى التحقق من البيانات المرسلة.");
      } else if (error.message.includes("HTTP error! status: 500")) {
        setMessage("خطأ في الخادم (500). يرجى المحاولة مرة أخرى لاحقًا أو التواصل مع الدعم.");
      } else {
        setMessage("حدث خطأ. يرجى المحاولة مرة أخرى: " + error.message);
      }
    } finally {
      setLoading(false);
    }
